...
export async function mountPrivateAccount(host,bridge,connect=connectFirebase){
  ...
  async function account(user){const epoch=++generation;clearTimeout(timer);stopWatching?.();stopWatching=null;unsubscribe?.();unsubscribe=null;session?.close();session=null;bridge.leaveAccount();bridge.sessionChanged?.(user,api);$('[data-member]').hidden=!user;$('[data-login]').hidden=!!user;$('[data-verified]').hidden=true;verificationUI();if(!user){message('Signed out. Device-only records remain separate.');return;}$('[data-identity]').textContent=user.email;$('[data-verify]').hidden=user.emailVerified;$('[data-refresh]').hidden=user.emailVerified;if(!user.emailVerified){message('Email is not verified yet. Open the verification link from your inbox, then check verification status here. Device-only records remain available.');return;}
    ...
    for(const selector of ['[data-import]','[data-online]','[data-backup]'])$(selector).disabled=false;
    $('[data-sync]').textContent='Sync now';
    let openFailed=false;try{await active.open();}catch{openFailed=true;}if(epoch!==generation)return;bridge.setLoading(false);
    if(openFailed&&!active.state.payload){active.close();session=null;bridge.leaveAccount();for(const selector of ['[data-import]','[data-online]','[data-backup]'])$(selector).disabled=true;$('[data-sync]').textContent='Retry account connection';message('Your email is verified, but the online account could not load. Your saved device records remain available in the tracker. Retry the connection when ready.');return;}
  ...
  $('[data-sync]').onclick=safely(async()=>{if(!session&&api.auth.currentUser?.emailVerified){await account(api.auth.currentUser);return;}if(session?.state.dirty)await session.sync();else await session?.loadOnline();});
