...
export function buildReport(data,from,to,{includeNotes=false,timezone=Intl.DateTimeFormat().resolvedOptions().timeZone}={}){
  ...
  for(let date=from;date<=to;date=new Date(Date.parse(date+'T12:00:00Z')+86400000).toISOString().slice(0,10)){
    ...
    for(const {s,time} of items){const r=recordFor(data,s.id,date,time);rows.push({date,time,intakeDay:!!s.intakeDay,scheduledDate:s.intakeDay&&time<'06:00'?new Date(Date.parse(date+'T12:00:00Z')+86400000).toISOString().slice(0,10):date,medicine:s.name,dose:s.dose,asNeeded:!!s.asNeeded,status:r?.status||'unknown',actual:r?.actual||null,note:includeNotes?r?.note||'':'',corrections:r?.revisions?.length||0});}
  }
  if(rows.length>15000)throw Error('This report is too large. Choose fewer days.');
  return {version:1,patient:cleanProfile(data.profile).name,from,to,timezone,generatedAt:new Date().toISOString(),rows,remarks,lockedDates};
}
...
export function renderReport(host,report,{includeDetails=true}={}){
  ...
  if(report.lockedDates?.length)add('p',report.lockedDates.length+' day(s) saved and locked by the patient. Locking preserves the entered record; it is not clinical verification.');
  if(report.remarks.length){add('h3','Daily notes for discussion');for(const r of report.remarks)add('p',r.date+' · '+(r.kind==='correction'?'Correction added '+new Date(r.recordedAt).toLocaleString()+': ':r.kind==='doctorAdvice'?'Doctor advice (patient note): ':'')+r.text);}
